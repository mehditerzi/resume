...
$id = $_GET['id'];
$selected = $db->query("SELECT * FROM products WHERE id = '{$id}'")->fetch(PDO::FETCH_ASSOC);
if ( $selected ){
}
?>
<html lang="en">
                        ...
                        <div class="container" style="z-index: 100;">
                            <div class="row">
                                <select class="form-control" id="categories" onchange="getbrands()">
                                    ...
                                    if ( $query->rowCount() ){
                                        foreach( $query as $row ){
                                            ?>
                                            <option value="<?php print $row['id'];?>" <?php if ($selected['category_id']==$row['id']){print "selected";
                                            $category = $row['id'];} ?>><?php print $row['name']?></option>
                                            <?php
                                        }
                                    }
                                    ?>
                                ...
                                <select class="form-control" id="brands">
                                    ...
                                    <?php
                                    $query = $db->query("SELECT * FROM brands WHERE category_id=".$category, PDO::FETCH_ASSOC);
                                    if ( $query->rowCount() ){
                                        foreach( $query as $row ){
                                            ?>
                                            <option value="<?php print $row['id'];?>" <?php if ($selected['brand_id']==$row['id']){print "selected";} ?>><?php print $row['name']?></option>
                                            <?php
                                        }
                                    }
                                    ?>
                            ...
                            <div class="row">
                                <div class="col">
                                    ...
                                    <input placeholder="Ürün adı giriniz" id="name" type="text" class="form-control" value="<?php print $selected['name'];?>">
                                </div>
                                <div class="col">
                            ...
                            <div class="row" id="border">
                                <textarea id="description" style="width: 200px;"><?php print $selected['description'];?></textarea>
                            </div>
                            <div class="row">
                                <div class="col">
                                    <label for="barcode">Barkod No</label>
                                    <input id="barcode" type="text" class="form-control" value="<?php print $selected['barcode'];?>">
                                </div>
                                <div class="col">
                                    <label for="price">Satış Fiyatı</label>
                                    <input id="price" type="number" step="0.5" class="form-control" value="<?php print $selected['price'];?>" >
                                </div>
                                <div class="col">
                                    <label for="price">Vergisiz Alış Fiyatı</label>
                                    <input id="taxlessbuyprice" type="number" step="0.5" class="form-control" value="<?php print $selected['buy_price'];?>" >
                                </div>
                                <div class="col">
                                    <label for="price">Vergili Alış Fiyatı</label>
                                    <input id="taxedbuyprice" type="number" step="0.5" class="form-control" value="<?php print $selected['taxedbuy_price'];?>" >
                                </div>
                                <div class="col">
                                    <label for="price">Vergi Yüzdesi</label>
                                    <input id="tax" type="number" step="0.5" class="form-control" value="<?php print $selected['tax'];?>" >
                                </div>
                                <div class="col">
                                ...
                                <div class="col">
                                    ...
                                    <input id="saleprice" disabled type="number" step="0.5" class="form-control" value="<?php print $selected['saleprice'];?>">
                                </div>
                            </div>
                            <div class="row">
                                <div class="col">
                                    <label>Stok Sayısı</label>
                                    <input type="number" step="1" class="form-control" id="quantity" value="<?php print $selected['quantity'];?>">
                                </div>
                                <div class="col">
                                    <button onclick="checkform()" class="btn btn-primary" style="float: right;margin-top: 30px;">Kaydet</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function addproduct() {
        ...
        $.ajax({
            type: 'POST',
            url: 'assets/php/saveproduct.php',
            processData: false,  // tell jQuery not to process the data
            contentType: false,  // tell jQuery not to set contentType
            data: data,
            success: function (output) {
