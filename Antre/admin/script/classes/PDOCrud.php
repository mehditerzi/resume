    ...
    private function initializeBtnActions() {
        if ($this->settings["actionbtn"]) {
            if ($this->settings["viewbtn"]) {
                $this->enqueueBtnActions("view", "javascript:;", "view", "<i class=\"fa fa-2x fa-info-circle\"></i>", "", array("title" => $this->langData["view"]), "btn-info");
            }
            if ($this->settings["editbtn"]) {
                $this->enqueueBtnActions("edit", "javascript:;", "edit", "<i class=\"fa fa-2x fa-pencil-square-o\" style='margin-left: 5px;'></i>", "", array("title" => $this->langData["edit"]),"btn-warning");
            }
            if ($this->settings["inlineEditbtn"]) {
                $this->enqueueBtnActions("inline_edit", "javascript:;", "inline_edit", "<i class=\"fa fa-2x fa-pencil\"></i>", "", array("title" => $this->langData["inline_edit"]),"btn-warning");
            }
            if ($this->settings["delbtn"]) {
                $this->enqueueBtnActions("delete", "javascript:;", "delete", "<i class=\"fa fa-2x fa-times fa-fw\"></i>", "", array("title" => $this->langData["delete"]),"btn-danger");
            }
            if (isset($this->settings["clonebtn"]) && $this->settings["clonebtn"]) {
                $title = isset($this->langData["clone"]) ? $this->langData["clone"] : "sffd";
                $this->enqueueBtnActions("clone", "javascript:;", "clone", "<i class=\"fa fa-2x fa-clone\"></i>", "", array("title" => $title),"btn-warning");
            }
        }
    }
